...
- Exactly one source of truth for the cookie name string: the shared constant in `src/lib/authCookies.ts`. No literal `"refreshToken"`/`"refreshTokens"` strings may remain in any other file.
